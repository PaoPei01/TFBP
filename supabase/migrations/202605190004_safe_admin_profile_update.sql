...
begin
  ...
  set
    email = coalesce(nullif(input_new_data->>'email', ''), email),
    student_id = coalesce(nullif(input_new_data->>'student_id', ''), student_id),
    name_th = coalesce(nullif(input_new_data->>'name_th', ''), name_th),
    name_en = coalesce(nullif(input_new_data->>'name_en', ''), name_en),
    nickname = coalesce(input_new_data->>'nickname', nickname),
    major = coalesce(nullif(input_new_data->>'major', ''), major),
    ...
    admission_round = coalesce(nullif(input_new_data->>'admission_round', ''), admission_round),
    form_submitted_at = coalesce((nullif(input_new_data->>'form_submitted_at', ''))::timestamp with time zone, form_submitted_at),
    registration_order = coalesce((nullif(input_new_data->>'registration_order', ''))::integer, registration_order),
    ...
    public_profile = coalesce((nullif(input_new_data->>'public_profile', ''))::boolean, public_profile),
    show_instagram = coalesce((nullif(input_new_data->>'show_instagram', ''))::boolean, show_instagram),
    show_line_id = coalesce((nullif(input_new_data->>'show_line_id', ''))::boolean, show_line_id)
