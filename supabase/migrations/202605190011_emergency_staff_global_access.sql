...
using (
  ...
  or exists (
    ...
    where sa.user_id = auth.uid()
      ...
      and (
        sa.role = 'emergency_staff'
        or (
          sa.main_group = ga.main_group
          and (
...
begin
  ...
  return jsonb_build_object(
    ...
    'can_view_staff', is_admin_user or exists (
      select 1 from public.staff_assignments where user_id = auth.uid() and role in ('staff','mentor','viewer','emergency_staff')
    ),
